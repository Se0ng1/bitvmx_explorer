...
export const detectNextWinternitz = (code) => {
    ...
    if (matchWinternitz && matchWinternitz.length > 0) {
        ...
        const regexWordEnd = /6c768f.*?93.*?9d/;;
...
export const computeWinternitzWord = (digits, d0) => {
    ...
    } while (checksumValue === null || computedChecksum === null || (checksumValue != computedChecksum && amountOfDigitsChecksum <= valueDigits.length))
...
export const decryptWinternitzWitness = (witness, code) => {
    let pendingWitnessDigits = witness.filter((_, index) => index % 2 === 1).map((item) => (item === '' ? '00' : item));
    let pendingWitnessKeys = witness.filter((_, index) => index % 2 === 0).map((item) => (item === '' ? '00' : item));
    let resultArray = []
    let currentCode = code;
    while (pendingWitnessDigits.length > 0) {
        let nextWinternitzProps = detectNextWinternitz(currentCode);
        const amountOfDigits = [...currentCode.slice(0, nextWinternitzProps.endWord).matchAll('a3766b6b(76a9)+6c')].length
        let currentWitnessDigits = pendingWitnessDigits.slice(pendingWitnessDigits.length - amountOfDigits, pendingWitnessDigits.length)
        let currentWitnessKeys = pendingWitnessKeys.slice(pendingWitnessDigits.length - amountOfDigits, pendingWitnessDigits.length).join('')
        ...
        const hash = CryptoJS.RIPEMD160(CryptoJS.SHA256(currentWitnessKeys).toString(CryptoJS.enc.Hex)).toString(CryptoJS.enc.Hex);
        ...
        pendingWitnessDigits = pendingWitnessDigits.slice(0, pendingWitnessDigits.length - amountOfDigits)
